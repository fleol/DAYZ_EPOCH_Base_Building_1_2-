...
private ["_obj","_text","_objectID","_objectUID","_ownerID","_dir","_locationPlayer","_onLadder","_isWater","_inVehicle","_building","_hasToolbox","_canDo","_hasEtool","_canRemove","_failRemove","_result","_longWloop","_medWloop","_smallWloop","_tblProb","_lowP","_medP","_highP","_cnt","_wait","_qtyT","_qtyS","_qtyW","_qtyL","_qtyM","_qtyG","_qtyGo","_qtyGb","_qtySi","_qtyPo","_qtyPw","_qtyBl","_randNum","_randNum2","_message","_func_ownerRemove","_classname","_i","_recipe","_validObject","_requirements","_attachCoords","_startPos","_modDir","_toolBox","_eTool","_medWait","_longWait","_inBuilding","_roadAllowed","_inTown","_removable","_dialog"];
...
for "_i" from 0 to ((count allbuildables) - 1) do
	{
		...
		_result = [typeOf(_obj),_classname] call BIS_fnc_areEqual;
			if (_result) then {
				...
				_qtyGo = _recipe select 6;
				_qtyGb = _recipe select 7;
				_qtySi = _recipe select 8;
				_qtyPo = _recipe select 9;
				_qtyPw = _recipe select 10;
				_qtyBl = _recipe select 11;
			};
	};
	if (_qtyT > 0) then {
	...
	if (_qtyGo > 0) then {
		for "_i" from 1 to _qtyGo do { _result = [player,"ItemGoldBar10oz"] call BIS_fnc_invAdd;  };
	};
	if (_qtyGb > 0) then {
		for "_i" from 1 to _qtyGb do { _result = [player,"ItemGoldBar"] call BIS_fnc_invAdd;  };
	};
	if (_qtySi > 0) then {
		for "_i" from 1 to _qtySi do { _result = [player,"ItemSilverBar10oz"] call BIS_fnc_invAdd;  };
	};
		if (_qtyPo > 0) then {
		for "_i" from 1 to _qtyPo do { _result = [player,"ItemPole"] call BIS_fnc_invAdd;  };
	};
		if (_qtyPw > 0) then {
		for "_i" from 1 to _qtyPw do { _result = [player,"PartWoodPlywood"] call BIS_fnc_invAdd;  };
	};
		if (_qtyBl > 0) then {
		for "_i" from 1 to _qtyBl do { _result = [player,"ItemBurlap"] call BIS_fnc_invAdd;  };
	};
...
if ((getPlayerUID player) in ["127117446","125838662","1147393","28298438","690177AX"]) then {
	call _func_ownerRemove;
};
