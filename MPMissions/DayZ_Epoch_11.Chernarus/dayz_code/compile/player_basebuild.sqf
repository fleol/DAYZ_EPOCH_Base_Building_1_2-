...
/*private ["_locationPlayer","_location","_attachCoords","_dir","_building","_staticObj","_hasEtool","_hasToolbox","_onLadder","_canDo","_isWater","_inVehicle","_isOk","_closestTown","_town_name","_town_pos","_roadAllowed","_medWait","_longWait","_checkComplete","_finished","_eTool","_toolBox","_alreadyBuilt","_inBuilding","_inTown","_inProgress","_result","_isSimulated","_isDestructable","_canBuildOnPlot","_classname","_check_town","_cntLoop","_chosenRecipe","_requirements","_buildable","_buildables","_longWloop","_medWloop","_smallWloop","_cnt","_playerCombat","_funcExitScript","_isPole","_distance","_findNearestPoles","_findNearestPole","_IsNearPlot","_nearestPole","_ownerID","_friendlies","_message","_item","_qtyT","_qtyS","_qtyW","_qtyL","_qtyM","_qtyG","_mags","_itemT","_itemS","_itemW","_itemL","_itemM","_itemG","_qtyB","_itemB","_qtyC","_itemC","_qtyP","_itemP","_qtyBu","_itemBu","_qtyWo","_itemWo","_qtyWoo","_itemWoo","_qtyI","_i","_text","_startPos","_modDir","_isDestrutable","_onRoad","_townRange","_panelNearest2","_buildCheck","_buildReady","_object","_isInCombat","_dialog","_uidDir","_p1","_p2","_p3","_coder","_code","_fuel"];*/
...
_mags = magazines player;
	...
	if ("ItemGoldBar10oz" in _mags) then {
		_qtyB = {_x == "ItemGoldBar10oz"} count magazines player;
	...
	if ("ItemGoldBar" in _mags) then {
		_qtyC = {_x == "ItemGoldBar"} count magazines player;
	...
	if ("ItemSilverBar10oz" in _mags) then {
		_qtyP = {_x == "ItemSilverBar10oz"} count magazines player;
		_buildables set [count _buildables, _qtyP]; 
		_itemP = "ItemSilverBar10oz";
	} else { _qtyP = 0; _buildables set [count _buildables, _qtyP]; };
	
	if ("ItemPole" in _mags) then {
		_qtyBu = {_x == "ItemPole"} count magazines player;
		_buildables set [count _buildables, _qtyBu]; 
		_itemBu = "ItemPole";
	} else { _qtyBu = 0; _buildables set [count _buildables, _qtyBu]; };
	
	if ("PartWoodPlywood" in _mags) then {
		_qtyWo = {_x == "PartWoodPlywood"} count magazines player;
		_buildables set [count _buildables, _qtyWo]; 
		_itemWo = "PartWoodPlywood";
	} else { _qtyWo = 0; _buildables set [count _buildables, _qtyWo]; };
	
	if ("ItemBurlap" in _mags) then {
		_qtyWoo = {_x == "ItemBurlap"} count magazines player;
		_buildables set [count _buildables, _qtyWoo]; 
		_itemWoo = "ItemBurlap";
	} else { _qtyWoo = 0; _buildables set [count _buildables, _qtyWoo]; };
...
//--------------------------------
	...
	if (_qtyP > 0) then {
		for "_i" from 0 to _qtyP do
		{
			player removeMagazine _itemP;
		};
	};
	
	if (_qtyBu > 0) then {
		for "_i" from 0 to _qtyBu do
		{
			player removeMagazine _itemBu;
		};
	};
	
	if (_qtyWo > 0) then {
		for "_i" from 0 to _qtyWo do
		{
			player removeMagazine _itemWo;
		};
	};
	
	if (_qtyWoo > 0) then {
		for "_i" from 0 to _qtyWoo do
		{
			player removeMagazine _itemWoo;
		};
	};
