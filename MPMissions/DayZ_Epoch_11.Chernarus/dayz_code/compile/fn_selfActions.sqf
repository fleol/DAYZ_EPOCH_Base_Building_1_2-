...
// ---------------------------------------Krixes Self Bloodbag Start------------------------------------
    ...
    if((speed player <= 1) && hasBagItem && _canDo) then {
        if (s_player_selfBloodbag < 0) then {
            s_player_selfBloodbag = player addaction[("<t color=""#c70000"">" + ("Self Bloodbag") +"</t>"),"scripts\selfbb\player_selfbloodbag.sqf","",5,false,true,"", ""];
        };
    } else {
...
_currentSkin = typeOf(player);
	...
	if (_currentSkin != globalSkin) then {
		...
		//player removeMPEventHandler ["MPHit", 0]; uncommented cause this line would break the Death Message Boards from Epoch
...
// ##### END #####
	...
	if(_isAlive) then {
		if(_isDestructable or _isWreck) then {
	...
	if (_isMan and !_isAlive and !_isZombie) then {
		if (s_player_studybody < 0) then {
			s_player_studybody = player addAction [localize "str_action_studybody", "scripts\study body\study_body.sqf",_cursorTarget, 0, false, true, "",""];
		};
	} else {
    ...
    if (s_clothes < 0) then {
            s_clothes = player addAction [("<t color=""#FF0000"">" + ("Take Clothes") + "</t>"), "scripts\remove\clothes.sqf",cursorTarget, 1, false, true, "",""];
        };
    } else {
